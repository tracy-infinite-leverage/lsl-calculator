...
import { calculateWA, calculateWASafe } from '../index';
...
describe('WA gold-standard suite — single mode', () => {
  for (const fx of fixtures) {
    describe(`${fx.name} — ${fx.title}`, () => {
      const result =
        fx.input.trigger.kind === 'cash_out'
          ? calculateWASafe(fx.input.employee, fx.input.trigger)
          : calculateWA(fx.input.employee, fx.input.trigger);
