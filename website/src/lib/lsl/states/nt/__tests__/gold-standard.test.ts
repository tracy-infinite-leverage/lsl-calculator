// NT engine gold-standard fixture runner — T9.1 scaffold.
// Audit trail: docs/qa/test-cases-nt.md (PM-signed 2026-05-27).
// Phase 9 plan: docs/learnings/phase-state-engine-pattern.md.
...
describe('NT gold-standard suite — single mode', () => {
  for (const fx of fixtures) {
    describe(`${fx.name} — ${fx.title}`, () => {
      // NT s.10(4) hard-error path: cash_out triggers throw NTCashOutForbiddenError,
      // captured by calculateNTSafe.
      const result =
        fx.input.trigger.kind === 'cash_out'
          ? calculateNTSafe(fx.input.employee, fx.input.trigger)
          : calculateNT(fx.input.employee, fx.input.trigger);
