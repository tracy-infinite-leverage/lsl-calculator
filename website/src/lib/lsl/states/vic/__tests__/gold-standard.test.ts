...
import { calculateVIC, calculateVICSafe } from '../index';
...
interface Fixture {
  ...
  expected: {
    ...
    variance?: string;
    /** For failed cash-out fixtures. */
    errorCode?: string;
    /** Tolerance for years-of-service (4dp default). */
    yearsTolerance?: number;
  };
}
...
describe('VIC gold-standard suite — single mode', () => {
  for (const fx of fixtures) {
    describe(`${fx.name} — ${fx.title}`, () => {
      // For cash-out / failed-status fixtures, use the safe wrapper so the
      // VICCashOutProhibitedError is captured into the result.
      const result =
        fx.input.trigger.kind === 'cash_out'
          ? calculateVICSafe(fx.input.employee, fx.input.trigger)
          : calculateVIC(fx.input.employee, fx.input.trigger);
