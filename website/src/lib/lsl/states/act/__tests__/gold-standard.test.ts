...
import { calculateACT, calculateACTSafe } from '../index';
...
describe('ACT gold-standard suite — single mode', () => {
  for (const fx of fixtures) {
    describe(`${fx.name} — ${fx.title}`, () => {
      const result =
        fx.input.trigger.kind === 'cash_out'
          ? calculateACTSafe(fx.input.employee, fx.input.trigger)
          : calculateACT(fx.input.employee, fx.input.trigger);
