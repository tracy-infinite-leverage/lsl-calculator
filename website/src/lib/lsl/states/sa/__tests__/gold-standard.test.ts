...
import { calculateSA, calculateSASafe } from '../index';
...
describe('SA gold-standard suite — single mode', () => {
  for (const fx of fixtures) {
    describe(`${fx.name} — ${fx.title}`, () => {
      const result =
        fx.input.trigger.kind === 'cash_out'
          ? calculateSASafe(fx.input.employee, fx.input.trigger)
          : calculateSA(fx.input.employee, fx.input.trigger);
