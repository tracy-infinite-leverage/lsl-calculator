...
import { calculateQLD, calculateQLDSafe } from '../index';
...
interface Fixture {
  ...
  expected: {
    ...
    /** For failed cash-out fixtures (QLD does not currently produce these). */
    errorCode?: string;
  };
}
...
describe('QLD gold-standard suite — single mode', () => {
  for (const fx of fixtures) {
    describe(`${fx.name} — ${fx.title}`, () => {
      const result =
        fx.input.trigger.kind === 'cash_out'
          ? calculateQLDSafe(fx.input.employee, fx.input.trigger)
          : calculateQLD(fx.input.employee, fx.input.trigger);
