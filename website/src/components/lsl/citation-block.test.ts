...
describe('CitationBlock — byte-for-byte markup contract (spec §8.4)', () => {
  it('renders the bare-minimum single citation (section + rule, no pdfPage, no note)', () => {
    ...
    expect(html).toMatchInlineSnapshot(
      `"<ol class="space-y-2 mt-2" aria-label="Legislative citations"><li class="border-l-2 border-primary/40 pl-3 text-xs leading-relaxed"><div class="flex items-start gap-1.5"><svg class="h-3.5 w-3.5 text-primary mt-0.5 shrink-0" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" fill="none" aria-hidden="true"><use href="/icons/sprite.svg#default--book-open"></use></svg><div><p class="font-semibold text-foreground">NSW LSL Act 1955 s.4(2)</p><p class="text-muted-foreground font-mono text-[11px]">long_service_leave_entitlement_after_10_years</p></div></div></li></ol>"`,
    );
  });
  ...
  it('renders multiple citations in source order (visual order = source order)', () => {
    ...
    expect(html).toMatchInlineSnapshot(
      `"<ol class="space-y-2 mt-2" aria-label="Legislative citations"><li class="border-l-2 border-primary/40 pl-3 text-xs leading-relaxed"><div class="flex items-start gap-1.5"><svg class="h-3.5 w-3.5 text-primary mt-0.5 shrink-0" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" fill="none" aria-hidden="true"><use href="/icons/sprite.svg#default--book-open"></use></svg><div><p class="font-semibold text-foreground">NSW LSL Act 1955 s.4(2)</p><p class="text-muted-foreground font-mono text-[11px]">long_service_leave_entitlement_after_10_years</p></div></div></li><li class="border-l-2 border-primary/40 pl-3 text-xs leading-relaxed"><div class="flex items-start gap-1.5"><svg class="h-3.5 w-3.5 text-primary mt-0.5 shrink-0" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" fill="none" aria-hidden="true"><use href="/icons/sprite.svg#default--book-open"></use></svg><div><p class="font-semibold text-foreground">NSW LSL Act 1955 s.4(5)(c)</p><p class="text-muted-foreground font-mono text-[11px]">category_b_average_weekly_hours_lookback · LSL-training PDF p.14</p></div></div></li><li class="border-l-2 border-primary/40 pl-3 text-xs leading-relaxed"><div class="flex items-start gap-1.5"><svg class="h-3.5 w-3.5 text-primary mt-0.5 shrink-0" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" fill="none" aria-hidden="true"><use href="/icons/sprite.svg#default--book-open"></use></svg><div><p class="font-semibold text-foreground">NSW LSL Act 1955 s.4(5)(d)</p><p class="text-muted-foreground font-mono text-[11px]">category_c_five_year_average_floor · LSL-training PDF p.15</p><p class="text-muted-foreground italic mt-0.5">Cat C is the highest-of-three averages floor.</p></div></div></li></ol>"`,
    );
  });
