...
**Method-level overrides (sit outside the bucket table).** In addition to the bucket-level per-trigger variances above, some triggers override the whole averaging method or impose an additional payment obligation that is not just one bucket's treatment. Two known v1 cases:

1. **VIC death / illness-incapacity termination (s.10(3)(b))** switches the entire value-of-week calculation to a 52-week average immediately before death rather than the standard greater-of-12-months-or-5-years. This is method-level — every bucket inside the resulting window is sampled differently. Engine already implements it.

2. **SA cash-out variation top-up (LSL Act 1987 s.8(3a)(b)).** When LSL is cashed out under a written agreement, the rate of pay is fixed at the cash-out date. **If the employee's ordinary rate of pay rises during the period the cash-out payment covers** (e.g. award increase, EBA roll-over, promotion), the employer must make a **further payment** equal to the rate-difference × the affected portion of the cashed-out weeks. This is a statutory minimum, not an advisory — under-payment is a breach of the Act. The obligation applies only to the `cash_out` trigger in SA. Commission is still included for every SA trigger (taking_leave, termination_payout, as_at, cash_out) — the variation top-up is on top of, not instead of, the ordinary calculation. **Engine status**: implemented as of 2026-05-27 (PR #42 + PR #43). For fixed-rate FT / casual-156wk / higher-duties paths, the SA engine emits `sa_cashout_variation_topup_required` with rate at cash-out, rising rate(s) + effective dates, affected weeks, and the **computed top-up amount** whenever a qualifying rise is detected in `Employee.wageHistory` within the cash-out coverage window. Coverage length is driven by `extraInputs.sa_cashed_out_weeks` (falls back to full accrued `payableWeeks`). For commission-path workers (52-wk income average per SafeWork SA), the engine emits `sa_cashout_variation_topup_manual_reconcile_commission` instead — surfacing any rise above the substantive `currentWeeklyGross` with an indicative top-up amount, and flagging the legal ambiguity around "rate at cash-out" for commission workers (52-wk income average vs substantive base rate) for manual reconciliation.

Both overrides are bound by AC5.5.1a (paired-fixture rule) plus, for SA, AC5.5.1b (advisory surfacing rule).
...
- **AC5.5.1b** For an SA `cash_out` trigger where the persisted pay history shows the employee's ordinary rate of pay rising during the period the cash-out payment covers (from `cashOutDate` through `cashOutDate + cashed-out-weeks × 7` days), the platform MUST surface a clearly-labelled advisory in the valuation result citing SA LSL Act 1987 s.8(3a)(b). The advisory MUST include: the rate at the cash-out date, the rising rate(s) and their effective dates, the affected portion of the cashed-out weeks, and either the computed top-up amount or an explicit note that the top-up must be reconciled manually if the engine does not yet compute it. A SA cash_out result that omits the advisory when the wage history shows a qualifying rate rise is a Sev-1 statutory-minimum-underpayment defect. **Note**: the underlying engine warning emission (`sa_cashout_variation_topup_required`) is out of scope for the auth slice (E5.1) and lands as a separate engine task; the platform-side requirement to *display whatever the engine emits* is in scope here.
