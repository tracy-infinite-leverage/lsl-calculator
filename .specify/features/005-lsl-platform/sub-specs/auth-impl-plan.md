...
| 2026-05-26 | Auth pages live under `src/app/app/...` in the existing Next.js 16 App Router. Single `src/middleware.ts` at the website root. | Phase 1 |
...
|---|---|---|---|
| DEV-AUTH-1 | Confirm R1 (`@supabase/ssr` on Next.js 16 + React 19) before WP 5.1. If incompatible, fall back to `@supabase/auth-helpers-nextjs` and log the divergence. | Developer | Yes — blocks 5.1 |
| DEV-AUTH-2 | Confirm R2 (trigger atomicity) before WP 4.4. If trigger pattern fails, use server-route + service-role transaction as fallback. | Developer | Yes — blocks 4.4 |
| DEV-AUTH-3 | Confirm R3 (HIBP toggle in Supabase Auth dashboard) before WP 9.2. If unavailable, implement server-side HIBP k-anon API call in signup + reset-password actions. | Developer | No — fallback is straightforward |
