...
**Status**: Draft v1 — for PM sign-off
**Date**: 2026-05-21
**Spec**: spec.md v0.4.1
...
total_entitlement_weeks: TBD                      # snapshot accrual — see Notes
total_entitlement_dollars: TBD
expected_citations:
...
employee:
  ...
  statesOfService: [NSW]
  governingJurisdiction: NSW
  currentHourlyRate: 37.50          # PDF: "$37.50 per hour incl casual loading"
  currentWeeklyGross: TBD            # see Notes
  hoursLast12Months: 1613.33         # PDF verbatim
  wageHistory:
...
system_formula_value: TBD  # $37.50 × 1613.33/52 × 3 = ≈ $3,489.30 — system rounding model PM-signable
variance:                TBD
```
...
expected_per_row_assertions: TBD            # generated by the fixture builder; checksum-asserted
```

**Notes**

Per-row assertions are TBD — `expected: TBD — fixture builder script populates expected dollars per row from the same engine; the test asserts performance + structural completeness rather than per-row legislated value (that's covered by TC-NSW-001 … TC-NSW-026)`. PM to sign off that the 50-row fixture is a perf-only gate, not a correctness gate (correctness is covered by the 60 enumerated cases above).
...
# Items flagged `expected: TBD — confirm with PM`

The following cases carry a TBD on a sub-field; each is annotated inline. Tracy to review and resolve before sign-off:

1. **TC-NSW-006 (Michelle)** — total_entitlement_weeks/dollars in `as_at` mode at 6.99 yrs of service. The PDF stops at value-of-week; engine output depends on whether snapshot shows "accrued so far" regardless of milestone (per D20 — recommended) or shows "$0 until 10 yrs" (per s.4(2) take-leave rule). Recommended: snapshot per D20.
2. **TC-NSW-007 (Liam taking 3 wks)** — `system_formula_value` and `variance` for the F21 display. The system-formula computation `hourly × hours/52 × weeks` returns approximately $3,489.30 vs. legislated $3,480.81 — small variance ($8.49). PM to confirm the system-formula model.
3. **TC-NSW-009 (Rinaldo)** — leave amount taken (PDF doesn't state). Test fixture assumes full entitlement; adjust if PM wants partial.
4. **TC-NSW-020 (Kate Practice 1A)** — bonus is v1-out. Should the engine reject Kate's input with a warning, or compute the no-bonus path silently? Recommended: warning if wage notes mention "bonus" / "incentive".
5. **TC-NSW-024 (12-yr casual-to-FT)** — `employmentTypeHistory` is not in the v1 `Employee` schema. PM to confirm that v1 takes a single current `employmentType` and the classifier resolves the pay-pattern from wage history (impl-plan §3.3).
6. **TC-NSW-036 (serious misconduct at 12 yrs)** — PM to confirm reading of PDF p.25 "10+ years — any reason" overrides the serious-misconduct exclusion that applies at 5-to-<10.
7. **TC-NSW-044** — "within 2 months" — inclusive or exclusive of the boundary? Recommended: inclusive (≤ 60 days).
8. **TC-BULK-003** — 50-row fixture treated as perf-only or correctness-also? Recommended: perf-only — correctness is covered by enumerated cases.

---

*End of test-cases.md v1.*
