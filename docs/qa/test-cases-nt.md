...
| 7–10-year qualifying reasons | **retirement (Age Pension age), employer-not-misconduct (including `unfair_dismissal`), illness/incapacity/domestic-pressing-necessity** — death NOT in this list (covered separately under s.10(3)) | s.10(2) |
...
| `nt_per_year_formula_applied` | informational | Per-year `RP × HWW × 1.3` summation applied per s.11(3). Fires unconditionally whenever the per-year branch is taken — including the fallback path (no operator history → 38 hr/wk virtual bucket) and the partial-history path. Data-quality concerns are communicated separately via `nt_per_year_hours_history_missing` / `_partial`. |
...
| `nt_industrial_dispute_excluded` | informational | Industrial dispute time does NOT count as service per s.12 unless worker returns under settlement. Fires only on worker-led `industrial_action` events (the s.12 "industrial dispute does not count" provision). Does NOT fire on `employer_stand_down` events — those preserve continuity (but not length) under the slackness-of-trade clause and are handled by the separate stand-down treatment per signed spec line 152. |
...
- **Pro-rata at termination** (s.10(2)): payable to an employee who has completed at least **7 years** of continuous service whose employment ends for a qualifying reason — retirement (Age Pension age — TBD-NT-02), **employer-not-misconduct dismissal (read to include `unfair_dismissal`, which by definition is employer-initiated and not for misconduct — cross-state precedent across NSW/VIC/QLD/WA/SA/ACT/TAS; mirrors TAS T8.3 reconciliation Item 1 ruling 2026-05-26)**, illness/incapacity/domestic-pressing-necessity. **Voluntary resignation 7–10 yrs does NOT qualify** (TBD-NT-04). **Death is NOT named in s.10(2)** — covered separately under s.10(3) (TBD-NT-06). Pro-rata calculation includes years AND months/12 converted to decimal.
...
- **Misconduct block-truncation worked examples** (s.10(1A) implementation per TBD-NT-05): the engine applies `truncateToCompleted10Or15YrBlock(yrs) = 10 + floor((yrs−10)/5) × 5` for yrs ≥ 10; sub-10-yr returns $0 forfeiture. **Examples**: 9.5y → $0 (sub-10 forfeiture); 10.0y → 10y block; 12.5y → 10y block; 16y → 15y block; 21y → 20y block; 29.99y → 25y block. Only complete 10y/15y/20y/25y blocks payable.
...
- **NT per-year `valueOfWeek` is an aggregate average** (engine convention, not statutory): the cross-state `Result.outputs.valueOfWeek: Decimal` schema is a single value, while NT s.11(3) is structurally a sum. The engine reports `valueOfWeek = totalDollars / payableWeeks` so that `valueOfWeek × payableWeeks = totalDollars` is exact by construction. When per-year HWW varies (e.g. 20→30→38 hr/wk across years), the surfaced `valueOfWeek` is a blended figure across actual hours-worked. This is the correct s.11(3) surface — operator should read the `totalDollars` figure as the load-bearing output, not `valueOfWeek` in isolation. **Worked example**: TC-NT-058 (13 yrs, 20→30→38 hr/wk, current rate $2000) produces `totalDollars = $30,247.44`, `payableWeeks = 16.9027`, `valueOfWeek = $1,789.51` (blended).
...
## Reconciliation amendments — 2026-05-27 (T9.3)

The developer's T9.3 fixture-corpus build (78 fixtures) surfaced 9 items where engine output diverged from doc-spec'd expected values or where the doc itself contained ambiguities worth recording. PM ruled on each (2026-05-27) as part of the T9.3 reconciliation pass. **Doc remains PM-SIGNED — no re-sign required.** Amendments are non-blocking and confined to the rulings table below plus seven targeted clarifications above (Amendments 1–7).

| # | Topic | Ruling | Action |
|---|---|---|---|
| 1 | `unfair_dismissal` already in NT `QUALIFYING_REASONS` set (T9.2) — but doc Quick-reference + Conventions did not name it | **(D) Amend signed doc** | Cross-state precedent (NSW/VIC/QLD/WA/SA/ACT/TAS all wire `unfair_dismissal`); TAS T8.3 Item 1 set the parallel. Doc-only Amendments 1 + 2 added clarifying that "employer-not-misconduct" reads to include `unfair_dismissal`. Engine unchanged — already correct. |
| 2 | `nt_hours_per_week_by_year` fallback path semantics (38 hr/wk virtual bucket with rate `currentWeeklyGross / 38`) not documented at the schema row | **(D) Amend signed doc** | Engine correct per locked TBD-NT-01 — the flat-path product `currentWeeklyGross × years × 1.3` is preserved by construction. Doc-only Amendment 5 added the implementation note. Engine unchanged. |
| 3 | `nt_per_year_formula_applied` fires on the fallback path (no operator history) — apparent contradiction with the warning's framing as "per-year sum applied" | **(D) Amend signed doc** | Engine intentional: the per-year branch IS taken on the fallback (single virtual bucket); data-quality is communicated separately via `nt_per_year_hours_history_missing`. Doc-only Amendment 6 added the clarification. Engine unchanged. |
| 4 | `nt_industrial_dispute_excluded` fires only on worker-led `industrial_action` events, not on `employer_stand_down` | **(D) Amend signed doc** | Engine correct per s.12 (worker industrial dispute does not count; stand-down for slackness preserves continuity but not length). Doc-only Amendment 7 added the clarification. Engine unchanged. |
| 5 | `unfair_dismissal` qualifying-reason wiring location | **(A) Engine fix already applied at T9.2** | Verified `unfair_dismissal` present in NT `QUALIFYING_REASONS` at `nt/rules/accrual-table.ts:89`. Cross-state parity with TAS T8.3 Item 1. No new engine work. |
| 6 | s.10(1A) misconduct block-truncation formula `10 + floor((yrs−10)/5) × 5` — worked examples not in signed doc | **(D) Amend signed doc** | Engine correct per locked TBD-NT-05. Doc-only Amendment 4 added the formula + 6 worked examples (9.5y / 10.0y / 12.5y / 16y / 21y / 29.99y). Engine unchanged. |
| 7 | TC-NT-034 / TC-NT-070 — related-corp aggregation surfaced a per-year sum that covered only PHYSICAL years, producing `valueOfWeek = currentWeeklyGross / 2` (halved) when half the aggregated tenure was related-corp | **(A) Engine fix** | Engine had a real gap: physical-year buckets summed correctly, but related-corp years contributed to `payableWeeks` (via accrual) without contributing dollars to the per-year sum. Fix in `nt/rules/value-of-week.ts`: append `nt_related_corporation_service_years` virtual full-tenure buckets at 38 hr/wk × `currentWeeklyGross / 38` rate after the physical buckets, capped by accrual headroom (misconduct truncation still wins). After fix: TC-NT-034 and TC-NT-070 produce `valueOfWeek = currentWeeklyGross` exactly, `totalDollars = currentWeeklyGross × aggregatedYears × 1.3` (modulo per-day fractional-year arithmetic). Existing `nt_related_corporation_service_aggregated` advisory is sufficient — no new warning code. |
| 8 | TC-NT-058 — varying hours (20→30→38 hr/wk over 13 yrs) produces blended `valueOfWeek = $1789.51`, not `currentWeeklyGross = $2000`. Counter-intuitive but engine-canonical | **(D) Amend signed doc** | Engine correct per s.11(3) — the per-year sum varies year-to-year by HWW; the surfaced `valueOfWeek = totalDollars / payableWeeks` is a blended average. Cross-state output shape forces a single `valueOfWeek` field. Doc-only Amendment 3 added the convention note + the TC-NT-058 worked example. Engine unchanged — `totalDollars = $30,247.44` is the load-bearing output. |
| 9 | Three fixtures (TC-NT-034, TC-NT-058, TC-NT-070) carried `_reconciliation_note` placeholder fields surfacing the reconciliation items | **(C) Fixture update** | After Item 7 engine fix + Items 1/2/3/4/6/8 doc amendments, all three fixtures fully reconcile. Remove `_reconciliation_note` from all three fixture JSONs; TC-NT-034 + TC-NT-070 receive new expected `valueOfWeek` / `totalEntitlementWeeks` / `totalEntitlementDollars` values matching the post-fix engine output (TC-NT-058 keeps the engine-canonical blended figure per Item 8 ruling). |

### Aggregate developer action list

- [x] Engine fix in `website/src/lib/lsl/states/nt/rules/value-of-week.ts` — append `relatedYears` virtual full-tenure buckets after the physical-year sum loop, with rate `currentWeeklyGross / 38`, hours-per-week = `38`, count = `extraInputs.nt_related_corporation_service_years` (default 0); capped by accrual headroom so misconduct truncation still wins (Item 7).
- [x] Verify `unfair_dismissal` is in NT `QUALIFYING_REASONS` at `accrual-table.ts:89` (already there from T9.2; no change) (Item 5).
- [x] Amendment 1 — Quick-reference table line 116: append `(including \`unfair_dismissal\`)` to the 7–10-yr qualifying-reasons row (Item 1).
- [x] Amendment 2 — Conventions s.10(2) bullet: expand "employer-not-misconduct dismissal" to read inclusively of `unfair_dismissal` with cross-state precedent (Item 1).
- [x] Amendment 3 — Conventions new bullet: `valueOfWeek` is an aggregate average; operator reads `totalDollars` as load-bearing; TC-NT-058 worked example (Item 8).
- [x] Amendment 4 — Conventions new bullet: s.10(1A) misconduct block-truncation worked examples (Item 6).
- [x] Amendment 5 — Schema row Purpose cell: append fallback-path implementation note (38 hr/wk virtual bucket; rate `currentWeeklyGross / 38`; display-only artefact) (Item 2).
- [x] Amendment 6 — Warning row Use cell: append `nt_per_year_formula_applied` fires unconditionally on per-year branch (Item 3).
- [x] Amendment 7 — Warning row Use cell: append `nt_industrial_dispute_excluded` fires only on worker-led `industrial_action` (Item 4).
- [x] Fixture cleanup — remove `_reconciliation_note` from TC-NT-034 / TC-NT-058 / TC-NT-070; add post-fix expected outputs to TC-NT-034 + TC-NT-070; keep TC-NT-058 engine-canonical figure (Item 9).

### Sign-off status

Doc remains **PM-SIGNED 2026-05-27**. Reconciliation amendments are non-blocking — one targeted engine fix (Item 7), seven doc clarifications (Items 1, 2, 3, 4, 6, 8 mapped to Amendments 1–7), and three fixture cleanups (Item 9). No re-sign required. Developer proceeds with T9.3-apply commit incorporating the above.

---

*End of test-cases-nt.md v1.0 PM-SIGNED · Tracy Angwin (PM) · 2026-05-27 · Reconciliation appendix 2026-05-27 (T9.3).*
