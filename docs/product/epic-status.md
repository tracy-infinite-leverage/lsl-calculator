# LSL Calculator · Epic Status · Last updated: 2026-05-25 · E1 NSW SHIPPED (Phases 1+2+3 live on lsl-calculator.vercel.app) · E2 Phase 1+3+4 merged to main (PR #8 at `56ae5fd`, PR #10 at `93484f3`, PR #11 at `0bddbbc`, PR #13 QLD engine, PR #14 DEV-CROSS-1 `bd2d284` — VIC + QLD engines LIVE) · E2 Phase 5 T5.0 WA test-cases v1.0-draft on branch `pm/wa-test-cases` — awaiting PM sign-off and TBD-WA-01 (load-bearing dual-regime architecture) resolution
...
| E2 · All-State Coverage | 🔄 in flight | 60% | ●●●◐○ | 0 | 0 | Spec v0.3.1 + impl-plan v0.3.1 (Phase 4 acceptance criteria updated 2026-05-25) + tasks committed (10 phases, 76 tasks). **Phases 1+3+4 MERGED to `main`** (PR #8 `56ae5fd`, PR #10 `93854f3`, PR #11 `0bddbbc`, PR #13 QLD engine `11511fb`, PR #14 DEV-CROSS-1 `bd2d284`). NSW + VIC + QLD all LIVE — state selector, dispatcher, VIC engine (61 fixtures byte-identical), QLD engine (60 fixtures), VIC hard-error cash-out, QLD non-blocking cash-out advisory, product rename "LSL Calculator". **DEV-CROSS-1 (cross-state termination-reason enum + initiator)** merged at `bd2d284` (PR #14). **QLD test-cases v1.1 (2026-05-25)** — 5 previously deferred DEV-CROSS-1 fixtures (TC-QLD-005, -007, -008, -015, -016) reinstated as active QLD launch-gate fixtures and pass the engine gold-standard test on first run with no engine code changes; 57 active single-mode + 3 bulk = 60 total. |
...
- **Phase 5 T5.0 status**: 🔄 **DRAFTED 2026-05-25** by PM on branch `pm/wa-test-cases`. `docs/qa/test-cases-wa.md` v1.0-draft. **70 active single-mode + 3 bulk = 73 total fixtures** covering WA LSL Act 1958 ss.4–9 (post-2022 + pre-2022 regime split) + WCIM Act 2023 (WA) WC counts-from-2024-07-01 override. Coverage spans s.8(1) accrual, s.8(3) "any-reason except serious misconduct" pro-rata at 7+ yrs (WA divergence from NSW/QLD — voluntary resignation qualifies), the WA-unique 10+yr serious-misconduct PARTIAL forfeiture (only last fully-accrued block payable), s.9 accrual-period averaging (different from VIC 3-tier and QLD 52-wk), s.5 cash-out post-accrual advisory (non-blocking — aligns with QLD; contrast VIC s.34 hard error), s.6 dual continuous-service modules selected by accrual-block "fully accrued" date, the independent 2024-07-01 WC cutoff, the 2-mo/6-mo non-slackness/slackness re-employment tolerances, and 3 bulk fixtures asserting per-state branching across NSW + VIC + QLD + WA. **All 16 TBDs unresolved — pending PM decision before T5.1 unblocks**:
  - **TBD-WA-01** (Sev-1, **LOAD-BEARING** — dual-regime architecture): PM-recommended option (b) — single rule set with date-aware continuous-service handling, mirroring the resolved TBD-VIC-01 VIC re-scope. The 2022 amendment did NOT change the entitlement formula or pro-rata rules; it changed only continuous-employment rules (absence treatment, casual definition, parental leave, transfer-of-business scope). Two continuous-service-rule modules (pre-2022 + post-2022) feed the same s.8 accrual formula. Plus a third date-aware override at 2024-07-01 for WC. If PM accepts, impl-plan v0.3.1 §5 must be re-scoped — ~2 dev-days saved (5–8 → 4–6). If PM rejects in favour of impl-plan-default option (a) parallel rule sets, cross-cutoff fixtures TC-WA-040 → TC-WA-046 require re-derivation.
  - **TBD-WA-02 → TBD-WA-16** (Sev-2/3): WCIM Act 2023 section number; cash-out three-tier advisories; 15-yr accrual continuous/inclusive boundaries; WC rate of pay during LSL (no WA equivalent to VIC s.17); accrual-period partial-block averaging; 10+yr misconduct last-fully-accrued-block-less-leave-taken interaction; slackness-of-trade event-field schema extension; pre-2022 sickness cap working-days vs calendar-days; pre-2022 casual continuity general-rule application; cutoff inclusivity at 2022-06-20 exact; WC paid-concurrent/RTW schema extension; casual UPL reasonable-expectation schema extension; meals/accommodation cash value schema extension; pre-first-milestone cash-out hard-error vs advisory; sub-7-yr death no-carve-out.
- **Next action**: PM operator review of `docs/qa/test-cases-wa.md` v1.0-draft. After TBD-WA-01 (and other Sev-1) resolution, impl-plan v0.3.1 §5 update (if option (b) accepted), then T5.0 sign-off line populated and T5.1 (WA rule-set scaffold) unblocks. Phase 2 (mixed-state bulk CSV foundation) continues in parallel.
