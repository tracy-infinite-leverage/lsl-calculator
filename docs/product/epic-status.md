...
| E2 · All-State Coverage | 🔄 in flight | 50% | ●●●◐○ | 0 | 0 | Spec v0.3.1 + impl-plan v0.3.1 + tasks committed (10 phases, 76 tasks). **Phases 1+3 MERGED to `main`** (PR #8 `56ae5fd`, PR #10 `93854f3`, PR #11 `0bddbbc`). NSW + VIC both LIVE as of 2026-05-25 — state selector, dispatcher, VIC engine (61 fixtures byte-identical), VIC hard-error cash-out, product rename "LSL Calculator". **Phase 4 (QLD) T4.0 now drafted**: `docs/qa/test-cases-qld.md` v1.0-draft on branch `pm/qld-test-cases`, 63 test cases covering QLD IR Act 2016 ss.93–110 + s.134 + historical cliffs (s.96 = 23 Jun 1990, s.103 = 30 Mar 1994). **6 TBDs open** — PM sign-off required before T4.1 (QLD rule-set scaffold) can start. Next action: PM (Tracy) resolves TBDs and signs the document. |
...
- **Phase**: **Phase 3 (VIC) SHIPPED to `main`; Phase 4 (QLD) T4.0 DRAFT posted, awaiting PM sign-off** — 2026-05-25
- **Pipeline**: ●●●◐○ (Stage 1 Specified done; Stage 2 In flight — Phase 1+3 merged; Phase 4 T4.0 draft submitted)
...
- **Phase 4 T4.0 status**: ⏳ **DRAFT v1.0-draft posted 2026-05-25** by Tracy Angwin (PM). `docs/qa/test-cases-qld.md` on branch `pm/qld-test-cases`. 63 test cases covering QLD IR Act 2016 ss.93–110 (Chapter 2 Part 3 Division 9) + s.134 (general continuity) + historical cliffs (s.96 = 23 June 1990 general; s.103 = 30 March 1994 casual). Coverage spans the s.95(2) 10-yr automatic payout (incl. misconduct), the s.95(3)/(4) 7–10-yr qualifying-reason gate, s.110 cashing-out non-blocking advisory (vs VIC's hard error), s.105 casual loaded-rate, s.99 commission averaging, s.97 PH-exclusive treatment, and 3 bulk fixtures asserting per-state branching across NSW + VIC + QLD. **6 TBDs OPEN** (verbatim list — PM to resolve before T4.1):
  - **TBD-QLD-01** (Sev-1, 15-yr accrual + threshold inclusivity): is the 15-yr step discrete (10-14.99yr=8.6667 flat, jump to 13.0 at 15.0yr) or continuous (1/60 accrual throughout)? Are 7/10/15-yr thresholds inclusive at exact-day boundary?
  - **TBD-QLD-02** (Sev-1, historical cliffs s.96 + s.103): does s.96 (23 Jun 1990) hard-anchor service to 1990-06-23 or just emit advisory and use actual start date? Same for s.103 casual (30 Mar 1994). Recommendation in draft: hard-anchor s.103 casual cliff, advisory-only for s.96 (rarely operative in practice).
  - **TBD-QLD-03** (Sev-2, casual rate-of-pay averaging window): single 52-wk lookback per Business QLD's published formula, or multi-tier "greater of" like VIC s.15(2)? Draft assumes 52-wk.
  - **TBD-QLD-04** (Sev-2, cash-out advisory granularity): does engine require user to specify s.110 ground? Differentiate sub-10-yr cash-out advisory? Pass-through with $0 at sub-7-yr or refuse trigger? Draft says: no ground required, yes stronger advisory below 10-yr, pass through at sub-7-yr.
  - **TBD-QLD-05** (Sev-2, Workers Comp rate-of-pay): QLD has NO equivalent of VIC s.17 higher-of-rates. Apply literal s.98 ordinary-rate-at-leave-time even if reduced WC rate? Draft says yes; surfaces UX risk for operator review.
  - **TBD-QLD-06** (Sev-2, termination-reason enum design): add `terminationInitiator: 'employee' | 'employer'` sibling field? Add reason values `domestic_pressing_necessity`, `unfair_dismissal`? Disambiguate `incapacity_dismissal` vs `poor_performance`? Affects engine types, single-mode form UI. Same pattern recurs for WA/SA/TAS/ACT/NT.
- **Pre-flight blockers**: PM sign-off on `test-cases-qld.md` (T4.0) — currently DRAFT, 6 TBDs open. T4.1 (QLD rule-set scaffold) BLOCKED until signed.
- **Next action**: PM (Tracy) reviews `docs/qa/test-cases-qld.md` v1.0-draft, resolves 6 TBDs, signs the document. Operator then merges PR `pm/qld-test-cases` → main. T4.1 then unblocked for developer. Phase 2 (mixed-state bulk CSV foundation) continues in parallel where applicable.
