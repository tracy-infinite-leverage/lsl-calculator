# LSL Calculator · Epic Status · Last updated: 2026-05-25 · E1 NSW SHIPPED (Phases 1+2+3 live on lsl-calculator.vercel.app) · E2 Phase 1+3+4+5 merged to main (PR #8 `56ae5fd`, PR #10 `93484f3`, PR #11 `0bddbbc`, PR #13 QLD engine, PR #14 DEV-CROSS-1 `bd2d284`, PR #18 DEV-CROSS-2 `0a2d652`, PR #19 WA engine `fe66b99` — VIC + QLD + WA engines LIVE) · E2 Phase 6 (SA) T6.0 test-cases v1.0-DRAFT on branch `pm/sa-test-cases` — awaiting PM sign-off on 12 TBDs (1 Sev-1 dual-regime architecture, 2 Sev-2 schema-impact, 9 Sev-3)
...
| E2 · All-State Coverage | 🔄 in flight | 71% | ●●●●○ | 0 | 0 | Spec v0.3.1 + impl-plan v0.3.2 + tasks v0.3.2 committed (10 phases, 77 tasks; total 32–48 dev-days). **Phases 1+3+4+5 MERGED to `main`** (PR #8 `56ae5fd`, PR #10 `93854f3`, PR #11 `0bddbbc`, PR #13 QLD engine `11511fb`, PR #14 DEV-CROSS-1 `bd2d284`, PR #16 QLD v1.1 `fb52701`, PR #18 DEV-CROSS-2 `0a2d652`, PR #19 WA engine `fe66b99` — 73 WA fixtures incl. 5 DEV-CROSS-2-dependent reinstated). NSW + VIC + QLD + WA all LIVE. **Phase 6 (SA) T6.0 v1.0-DRAFT 2026-05-25** — `docs/qa/test-cases-sa.md` on branch `pm/sa-test-cases`, 67 fixtures (64 single + 3 bulk) covering SA LSL Act 1987 ss.3–6 + LSL (Calculation of Average Weekly Earnings) Amendment Act 2015 (SA). **12 TBDs surfaced**: TBD-SA-01 (Sev-1) dual-regime architecture — PM-recommended NO (2015 Amendment is forward-looking only, not a dual-regime cliff); TBD-SA-04 + TBD-SA-07 (Sev-2) schema-impact — TBD-SA-07 likely requires DEV-CROSS-3 (higher-duties acting-rate fields on `Employee`); other 9 (Sev-3) resolvable in implementation. T6.1 BLOCKED on operator resolution of the 3 critical-path TBDs. |
...
- **Phase**: **Phase 3 (VIC) SHIPPED; Phase 4 (QLD) SHIPPED (incl. DEV-CROSS-1 + QLD v1.1); Phase 5 (WA) SHIPPED (incl. DEV-CROSS-2 + WA engine T5.1-T5.5); Phase 6 (SA) T6.0 v1.0-DRAFT — awaiting PM sign-off on 12 TBDs; T6.1 BLOCKED on operator resolution of 3 critical-path TBDs (TBD-SA-01, TBD-SA-04, TBD-SA-07)** — 2026-05-25
- **Pipeline**: ●●●●○ (Stage 1 Specified done; Stage 2 In flight — Phase 1+3+4+5 merged; Phase 6 T6.0 v1.0-draft awaiting sign-off)
...
- **Phase 6 T6.0 SA status**: ⏳ **DRAFTED 2026-05-25** by Tracy Angwin (PM). `docs/qa/test-cases-sa.md` v1.0-DRAFT on branch `pm/sa-test-cases`. **64 active single-mode + 3 bulk = 67 total fixtures** covering SA LSL Act 1987 ss.3–6 (+ LSL (Calculation of Average Weekly Earnings) Amendment Act 2015 (SA) ordinary-pay methodology). Coverage spans: s.5(1) 13-week first entitlement at 10 yrs (F10/AC12 — most generous in Australia); s.5(3) 7-10-yr pro-rata with TWO disqualifiers (serious & wilful misconduct + SA-unique unlawful-worker-termination); s.5 PH-INCLUSIVE in LSL period (F11/AC13 — headliner divergence from all 4 prior states); SA-unique higher-duties acting rate (s.4); 156-wk all-hours casual averaging with WC/UPL substitution (s.4 as amended 2015); cashing-out non-blocking advisory post-10-yr (s.5); 10+ yr full-payout-regardless-of-reason (parallel to NSW/VIC/QLD; SA does NOT mirror WA's partial-forfeiture). **12 TBDs surfaced — awaiting operator decision**:
  - **TBD-SA-01** (Sev-1, LOAD-BEARING — dual-regime architecture): PM-recommended **NO**. LSL (AWE) Amendment Act 2015 (SA) applies prospectively to all LSL taken on/after commencement INCLUDING for absences before commencement — uniform forward-looking rule, not a dual-regime cliff. SA mirrors QLD's flat single-regime architecture. If confirmed NO, impl-plan v0.3.2 §6 stands and effort estimate stays at M (3-5 days).
  - **TBD-SA-04** (Sev-2, schema-impact — unlawful-worker-termination representation): PM-recommended `extraInputs.sa_worker_notice_compliance: boolean` (default `true`). Consistent with ACT extraInputs pattern (Phase 7).
  - **TBD-SA-07** (Sev-2, schema-impact — higher-duties acting rate): PM-recommended **DEV-CROSS-3** — promote `higherDutiesActive` + `higherDutiesWeeklyRate` to dedicated `Employee` fields (state-agnostic). Same pattern as DEV-CROSS-1 + DEV-CROSS-2. Effort estimate ½ dev-day.
  - **TBD-SA-02, -03, -05, -06, -08, -09, -10, -11, -12** (Sev-3, resolvable in implementation): casual continuity gap heuristic; cashing-out section reference (PM recommends Act-level citation pending verification — same documented-limitation precedent as TBD-WA-02); WC counts as service; cashing-out advisory granularity (PM recommends three codes parallel to WA); WC literal rate + advisory parallel to QLD/WA; single-day PH-LSL inclusive treatment; SA public-holidays calendar source; portable schemes out-of-scope; pre-1987 service moot for current calcs.
- **Next action**: Operator review of `docs/qa/test-cases-sa.md` v1.0-DRAFT. Resolve TBD-SA-01, -04, -07 (3 critical-path TBDs). PM applies resolutions and sign-off — file becomes v1.0 SIGNED OFF. If TBD-SA-07 resolved as PM-recommended, developer agent picks up DEV-CROSS-3 (state-agnostic higher-duties schema extension) as pre-flight for T6.1. T6.1 (SA rule-set scaffold) unblocks after sign-off + DEV-CROSS-3 merge.
